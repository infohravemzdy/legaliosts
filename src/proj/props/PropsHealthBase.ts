...
export abstract class PropsHealthBase extends PropsBase implements IPropsHealth {
  ...
  valueEquals(other: IPropsHealth): boolean {
    ...
    return (
      this.minMonthlyBasis === other.MinMonthlyBasis() &&
  ...
  roundedEmployeePaym(basisResult: number): number {
    ...
    return this.intInsuranceRoundUp(
      OperationsDec.multiplyAndDivide(new bigDecimal(basisResult), factorCompound, this.factorEmployee),
    );
  }
  ...
  roundedAugmentEmployeePaym(basisGenerals: number, basisAugment: number): number {
    ...
    return this.intInsuranceRoundUp(
      OperationsDec.multiply(new bigDecimal(basisAugment), factorCompound).add(
        OperationsDec.multiplyAndDivide(new bigDecimal(basisGenerals), factorCompound, this.factorEmployee),
      ),
    );
  }
  ...
  roundedAugmentEmployerPaym(basisGenerals: number, baseEmployee: number, baseEmployer: number): number {
    ...
    const compoundPayment = this.intInsuranceRoundUp(
      OperationsDec.multiply(new bigDecimal(compoundBasis), factorCompound),
    );
    const employeePayment = this.intInsuranceRoundUp(
      OperationsDec.multiply(new bigDecimal(baseEmployee), factorCompound).add(
        OperationsDec.multiplyAndDivide(new bigDecimal(basisGenerals), factorCompound, this.factorEmployee),
      ),
    );
  ...
  annualsBasisCut(
    incomeList: Iterable<ParticyHealthTarget>,
    annuityBasis: number,
  ): [number, number, Iterable<ParticyHealthResult>] {
    const annualyMaxim: number = this.maxAnnualsBasis;
    const annualsBasis = Math.max(0, annualyMaxim - annuityBasis);
    const resultInit: [number, number, Iterable<ParticyHealthResult>] = [
      annualyMaxim,
      annualsBasis,
      new Array<ParticyHealthResult>(),
    ];

    const resultList = Array.from<ParticyHealthTarget>(incomeList).reduce<
      [number, number, Iterable<ParticyHealthResult>]
    >((agr, x) => {
      ...
      const r = new ParticyHealthResult(
        x.contractCode,
        x.subjectType,
        x.interestCode,
        x.subjectTerm,
