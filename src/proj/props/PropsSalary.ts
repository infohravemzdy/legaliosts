...
export class PropsSalary extends PropsBase implements IPropsSalary {
  ...
  public static empty(): IPropsSalary {
    return new PropsSalary(VersionId.new(),
      0,
      0,
      0,
      0);
  ...
  private decPaymentWithMonthlyAndCoeffAndFullAndWorkHours(amountMonthly: bigDecimal, monthlyCoeff: bigDecimal, fullWorkHours: number, partWorkHours: number): bigDecimal {
  ...
  private decPaymentWithMonthlyAndFullAndWorkHours(amountMonthly: bigDecimal, fullWorkHours: number, partWorkHours: number): bigDecimal {
    const paymWorkHours = this.totalHoursWithFullAndPartHours(fullWorkHours, partWorkHours);
  
    const payment = OperationsDec.multiplyAndDivide(amountMonthly, new bigDecimal(paymWorkHours), new bigDecimal(fullWorkHours));
  ...
  paymentWithMonthlyAndFullWeekAndFullAndWorkHours(amountMonthly: bigDecimal,
    fullWeekHours: number, partWeekHours: number,
    fullWorkHours: number, partWorkHours: number): bigDecimal {
    const coeffSalary = this.coeffWithPartAndFullHours(new bigDecimal(partWeekHours), new bigDecimal(fullWeekHours));
  
    const salaryValue = this.paymentWithMonthlyAndCoeffAndFullAndWorkHours(amountMonthly, coeffSalary, fullWorkHours, partWorkHours);
  
    return salaryValue;
  }
  paymentRoundUpWithMonthlyAndFullWeekAndFullAndWorkHours(amountMonthly: bigDecimal,
    fullWeekHours: number, partWeekHours: number,
    fullWorkHours: number, partWorkHours: number): bigDecimal {
    const coeffSalary = this.coeffWithPartAndFullHours(new bigDecimal(partWeekHours), new bigDecimal(fullWeekHours));
  
    const salaryValue = this.paymentRoundUpWithMonthlyAndCoeffAndFullAndWorkHours(amountMonthly, coeffSalary, fullWorkHours, partWorkHours);
