...
describe('Service Salary For Year 2010-2022 WorkingShiftTime', () => {
  const scenarios = [
  ...
  logTestNumExamples('02_Salary_02_WorkingShiftTime.txt', scenarios);
    ...
    describe(`year ${tx.title}`, () => {
      tx.tests.forEach((tt) => {
        ...
        it(`GetProps should return value = ${tt.expected}`, () => {
          expect(props.WorkingShiftTime()).to.equal(tt.expected);
        });
      });
    });
  });
