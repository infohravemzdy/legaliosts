...
describe('Service Taxing For Year 2010-2022 MarginIncomeOfWithhold', () => {
  const scenarios = [
  ...
  logTestNumExamples('04_Taxing_17_MarginIncomeOfWithhold.txt', scenarios);
    ...
    describe(`year ${tx.title}`, () => {
      tx.tests.forEach((tt) => {
        ...
        it(`GetProps should return value = ${tt.expected}`, () => {
          expect(props.MarginIncomeOfWithhold()).to.equal(tt.expected);
        });
      });
    });
  });
