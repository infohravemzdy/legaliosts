...
describe('Service Taxing For Year 2010-2022 AllowanceDisab1st', () => {
  const scenarios = [
  ...
  logTestNumExamples('04_Taxing_02_AllowanceDisab1st.txt', scenarios);
    ...
    describe(`year ${tx.title}`, () => {
      tx.tests.forEach((tt) => {
        ...
        it(`GetProps should return value = ${tt.expected}`, () => {
          expect(props.AllowanceDisab1st()).to.equal(tt.expected);
        });
      });
    });
  });
