...
describe('Factory Health For Year 2011-2022', () => {
  class TestParams {
  ...
  const scenarios = [
    new TestScenario('2010', [
      new TestParams('2010-1', 2010, 1, 0),
      new TestParams('2010-2', 2010, 2, 0),
      new TestParams('2010-3', 2010, 3, 0),
      new TestParams('2010-4', 2010, 4, 0),
      new TestParams('2010-5', 2010, 5, 0),
      new TestParams('2010-6', 2010, 6, 0),
      new TestParams('2010-7', 2010, 7, 0),
      new TestParams('2010-8', 2010, 8, 0),
      new TestParams('2010-9', 2010, 9, 0),
      new TestParams('2010-10', 2010, 10, 0),
      new TestParams('2010-11', 2010, 11, 0),
      new TestParams('2010-12', 2010, 12, 0),
    ]),
    new TestScenario('2011', [
    ...
    describe(`year ${tx.title}`, () => {
      tx.tests.forEach((tt) => {
        describe(`getting props from factory for period ${tt.title}`, () => {
          const period: IPeriod = Period.getWithYearMonth(tt.year, tt.month);
          const sut = new FactoryHealth();
          const result = sut.GetProps(period);
          ...
          it(`GetProps should return GetVersionValue = ${tt.expected}`, () => {
            expect(result.Version().value).to.equal(tt.expected);
          });
        });
      });
    });
